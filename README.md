...
| `CLOUDINARY_CLOUD_NAME` |       | `your-cloud-name`                                           | Required in production for file uploads  |
| `CLOUDINARY_API_KEY` |          | `your-api-key`                                              | Required in production for file uploads  |
| `CLOUDINARY_API_SECRET` |        | `your-api-secret`                                           | Required in production for file uploads  |
...
| POST   | `/tasks/:id/attachments`    |  ✅  | Upload file (multipart, ≤10 MB). Dev mode saves locally; prod uses Cloudinary |
...
src/
  ...
  attachments/     file upload (Multer → Cloudinary), task scoped
...
### 12. File uploads (bonus)
`POST /tasks/:id/attachments` accepts multipart file uploads via Multer (max 10 MB). In development, files are saved locally; in production, they are streamed to Cloudinary and the URL is stored in the `attachments` table. The endpoint is JWT-protected and validates the owning task exists (deleted tasks rejected).

### 13. Iterative commit history
